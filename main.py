...
if(TRAINING):
    ...
    #! julianb
    F_initial_spikes = 10000*utils.gamma*np.divide(F_initial_spikes, np.sqrt(np.matmul(np.ones((2*utils.Nx,1)), np.sum(F_initial_spikes**2, axis=0).reshape((1,utils.Nneuron)))))
