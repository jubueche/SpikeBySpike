...
def runnet_recon_x(dt, lam, F, OT_up, OT_down, C, Nneuron, Ntime, Thresh, x_recon_lam = 0.001, x_recon_R = 1.0):
    ...
    x_recon = np.zeros((F.shape[0], 1)) # (2,1)
    
    for t in range(1, Ntime):
        V[:,t] = ((1-lam*dt)*V[:,t-1].reshape((-1,1)) + 0.1*np.matmul(F.T, x_recon.reshape((-1,1))) + np.matmul(C, O[:,t-1].reshape((-1,1))) + 0.001*np.random.randn(Nneuron,1)).ravel()
        x_recon = (1-0.001)*x_recon + x_recon_R*np.asarray([OT_up[0,t]-OT_down[0,t], OT_up[1,t]-OT_down[1,t]]).reshape((-1,1))
