...
#### Decoupling of decoder and encoding process.
# Given: Spike trains o(t)
# Compute rt from o(t) using rt = (1-utils.lambbda*dtt)*rt_1 + ot_1
eqs_decoupled = '''
rt : 1
rt_1 : 1
'''

G_decoupled = NeuronGroup(N=utils.N, model=eqs_decoupled, method='euler')
sm_G_decoupled = StateMonitor(G_decoupled, variables=True, record=True, dt=utils.delta_t*ms)

ot_delayed = np.copy(ot)
indices_decoupled, times_decoupled = np.where(ot_delayed == 1)

# Pick X random numbers from times_decoupled
num_samples = 100
n = len(times_decoupled)

random_subsamples = times_decoupled[np.random.randint(0,n,num_samples)]
random_subsamples_perm = np.zeros(len(random_subsamples), dtype = int)
for i in range(1,len(random_subsamples)):
        random_subsamples_perm[i-1] = random_subsamples[i]
random_subsamples_perm[-1] = random_subsamples[0]


for (i,j) in zip(random_subsamples,random_subsamples_perm):
        tmp = ot_delayed[:,i]
        ot_delayed[:,i] = ot_delayed[:,j]
        ot_delayed[:,j] = tmp

indices_decoupled, times_decoupled = np.where(ot_delayed == 1)
indices_swapped = np.asarray([indices_decoupled[i] for i in range(len(times_decoupled)) if(times_decoupled[i] in random_subsamples)]).flatten()
times_swapped = np.asarray([times_decoupled[i] for i in range(len(times_decoupled)) if(times_decoupled[i] in random_subsamples)]).flatten()


@network_operation(dt=utils.delta_t*ms)
def update_decoupled(t):
        current_t = int((t/ms)/utils.delta_t) # in [0,duration)
        rt = np.copy(np.reshape(G_decoupled.rt_, (-1,1)))
        rt_1 = np.copy(np.reshape(G_decoupled.rt_1_, (-1,1)))

        ot_1 = int(current_t>0)*ot_delayed[:,current_t-1]
        rt = (1-utils.lambbda*utils.dtt)*rt_1 + np.reshape(ot_1, (-1,1))

        G_decoupled.rt_ = np.copy(np.reshape(rt, (-1,)))
        G_decoupled.rt_1_ = np.copy(np.reshape(rt, (-1,)))

        
net_decoupled = Network(G_decoupled, sm_G_decoupled, update_decoupled)
net_decoupled.run(duration=utils.duration*ms)


x_hat_decoupled = np.matmul(D, sm_G_decoupled.rt_).T
...
if(penable):
        ...
        p5 = win.addPlot(title="Delayed-spike reconstruction")
        win.nextRow()

        p6 = win.addPlot(title="Delayed-spike trains")
        win.nextRow()
        ...
        p5.plot(y=x[:,1], pen=pg.mkPen('r', width=1, style=pg.QtCore.Qt.DashLine))
        p5.plot(y=x_hat_decoupled[:,1], pen=pg.mkPen('g', width=1, style=pg.QtCore.Qt.DashLine))

        p6.plot(x=times_decoupled, y=indices_decoupled,
                pen=None, symbol='o', symbolPen=None,
                symbolSize=3, symbolBrush=(68, 245, 255))
        p6.plot(x=times_swapped, y=indices_swapped,
                pen=None, symbol='o', symbolPen=None,
                symbolSize=5, symbolBrush=(245, 23, 45))
